...
class HymnsDatabase:
    ...
    def splitHymn(self, hymnFile: str):
        ...
        return HYMN(
            hymnFile[:2],
            hymnFile[3:6] if hymnFile[3:6].isdigit() else 0,
            hymnFile[7:-5],
            hymnFile.split(".")[-1],
        )
    ...
    def parseHymnDatabase(self):
        ...
        try:
            ...
            MSG_BOX.setWindowTitle("Error")
            MSG_BOX.setText(
                f"Database Error: Invalid contents.\nPlease contact the developers of this program to fix the issue."
            )
        ...
        for FILE in FNS:
            spH = self.splitHymn(
                FILE
            )  ## Splits the filename into separate parts using splitHymn()
            hNums.append(spH.num)  ## Append the hymn number to the dict 'Hymn Numbers'
            for i in range(3):  ## Loop for 3 categories (EN, TL, USER)
                if (
                    spH.cat == CATS[i] and spH.ext == "pptx"
                ):  ## Append all valid .pptx file into the EN, TL and USER dict
                    HYMNAL[i][0].append(spH.num)
                    HYMNAL[i][1].append(spH.title)
                    break
                elif (
                    spH.ext != "pptx" and FILE[-1] != "/"
                ):  ## Flags all non-pptx as unnecessary file
                    LOG.warn(f'Unnecessary file "{FILE}" detected inside the database.')

        ## Report Database Status
        MISSING = [
            KString.toDigits(i + 1, 3)
            for i in range(self.TOTAL_HYMNS)
            if KString.toDigits(i + 1, 3) not in hNums
        ]  ## Determines all missing hymns by looking through 1-474 range of hymns
        if MISSING:
        ...
        HYMNAL[3] = [
            [HYMNAL[i][0][j] + " " + HYMNAL[i][1][j] for j in range(len(HYMNAL[i][1]))]
            for i in range(3)
        ]  ## Append all hymns in this format: "<Hymn Number> <Hymn Title>" | Usually used for search completer
        HYMNAL[3] = [
            i for sl in HYMNAL[3] for i in sl
        ]  ## Flatten the nested list into one whole list
        HYMNAL[3].sort()
        HYMNAL[4] = _TOTAL(
            len(HYMNAL[0][0]), len(HYMNAL[1][0]), len(HYMNAL[2][0]), len(HYMNAL[3])
        )  ## Tuple of total hymns. Has EN, TL, USER, and ALL values.
        HYMNAL[5] = _MISSING(
            MISSING, len(MISSING)
        )  ## Tuple of missing hymns. Has list of missing hymns and length of the list.

        LOG.info(
            f"Successfully Scanned {HYMNAL[4].ALL} hymns in the database ({round((time.time()-_EXEC)*1000, 2)} ms)"
        )
    ...
    def getStats(self, hN):
        ...
        hN = KString.toDigits(
            hN, 3
        )  ## Converts the Hymn Number to a format of 3-digit padded hymn number
        if len(KString.filterOnly(0, hN)):
            return  ## Cancels the process when the hymn number is not a valid number or contains letters

        _HymnInfo = namedtuple(
            "HymnInfo",
            "title num eqTitle eqNum cat eqCat dbIndex queries launches lastAccessed lastAcssHumanize",
        )
        DATA = ["", hN, "", "", "", "", "", 0, 0, 0, 0]  ## Default values for HymnInfo
        idx = (
            0  ## Placeholder for dbIndex. 0 is still not recommended value for return.
        )
        ...
        if int(hN) > SYS.HYMNS_MAX or int(hN) <= 0 or hN in HYMNAL.MISSING.list:
            ...
            return _HymnInfo(
                ...
                DATA[2],
                DATA[3],
                DATA[4],  ## Returns placeholder data
                DATA[5],
                DATA[6],
                DATA[7],
                DATA[8],
        ...
        while True:
            try:
                hT = HYMNAL[i][1][HYMNAL[i][0].index(KString.toDigits(int(hN), 3))]
            except:
                if j:
                    DATA[0] = ""
                    break
                ...
                DATA[0] = hT
                break  ## Stores the first data (Title)

        ## Scan for Equivalent Hymn (Title and Number)
        i, j = (
            0,
            0,
        )  ## i is for the index iterations, j is for the lock switch; when j = 1, this indicates the final loop
        while True:
            if hN in HYMNAL[i][0]:
                try:
                    idx = HYMNAL[invert(i)][0].index(
                        KString.toDigits(operator.add(int(hN), -1 if i else 1), 3)
                    )
                except ValueError as e:
                    break  ## When the hymn can't be found on other category
                else:
                    DATA[2] = HYMNAL[invert(i)][1][
                        idx
                    ]  ## Stores the 3rd data (Equivalent Title)
                    DATA[3] = HYMNAL[invert(i)][0][
                        idx
                    ]  ## Stores the 4th data (Equivalent Number)
                    break
            else:
                if j:
                    break  ## breaks the search when hymn number doesn't match in both categories
                i, j = (
                    invert(i),
                    1,
                )  ## Switches to other [1] category while locked for last loop
        ...
        DATA[
            6
        ] = idx  ## Stores the 7th data (Index Number in Database; to reduce scanning again just to find the index)

        ## Get Stats of Base Hymn
        STS = (
            SDATA["DATA"][hN] if int(hN) or int(hN) <= SYS.HYMNS_MAX else [0, 0, 0]
        )  ## Statistic Data from Statistical Data (SDATA)
        DATA[7], DATA[8], DATA[9] = (
            STS[0],
            STS[1],
            STS[2],
        )  ## Stored the rest of data values (Queries, Launches, and Last Accessed)

        ## Return All Data
        if DATA[0] != "":
            UIZ.EXEC_READY = True  ## If the title, which is the most important, is available. Flag the execution in ready state.
        return _HymnInfo(
            DATA[0],
            DATA[1],
            DATA[2],
            ...
            DATA[4],
            DATA[5],
            DATA[6],  ## Returns a namedtuple of all data
            DATA[7],
            DATA[8],
            DATA[9],
            humanize.naturaltime(time.time() - DATA[9]),
        )
    ...
    def genSearchSuggestions(self, UI):
        ...
        self.CPLTR_SEARCH.setModel(MODEL)
        self.CPLTR_SEARCH.setCaseSensitivity(Qt.CaseInsensitive)
...
class Stylesheet:
    ...
    def initStylesheet(self):
        ...
        try:
            ...
            LST_PANEL_ICONS = [
                QtGui.QIcon(f"./res/icons/{name}_{self.THEME}.png")
                for name in ["settings", "hymnal", "library", "command"]
            ], [
                QtGui.QIcon(f"./res/icons/{name}_{self.THEME}.png") for name in ["info"]
            ]
            for i in range(2):
                for j, icon in enumerate(LST_PANEL_ICONS[i]):
                    UIB.LST_PANELS[i].item(j).setIcon(icon)
            self.ANIM_BTN_LAUNCH = ANM.ButtonAnimation(
                UIA.BTN_LAUNCH,
                modHex(self.PRIMARY, 30),
                self.PRIMARY,
                self.TXT_INV,
                self.TXT_INV,
            )
            self.ANIM_BTN_LAUNCH.connectEvents()
            self.ANIM_BTN_LAUNCH = ANM.ButtonAnimation(
                UIC.BTN_LAUNCH,
                modHex(self.PRIMARY, 30),
                self.PRIMARY,
            ...
            self.ANIM_BTN_LAUNCH.connectEvents()
            self.ANIM_BTN_RESET = ANM.ButtonAnimation(
                UIB.BTN_RESET,
                self.ERROR,
                self.palette2Hex("button"),
                self.palette2Hex("text"),
                self.palette2Hex("brightText"),
            )
            self.ANIM_BTN_RESET.connectEvents()
            self.ANIM_BTN_OK = ANM.ButtonAnimation(
                UIB.BTN_OK,
                modHex(self.PRIMARY, 30),
                self.PRIMARY,
                self.TXT_INV,
            ...
            self.ANIM_BTN_OK.connectEvents()
    ...
    def getStylesheet(self, objectName=None) -> str:
        """
    ...
    def toggleMode(self, mode):
        ...
        if not mode:
            ...
            PLT_LIGHT.setColor(
                QtGui.QPalette.Disabled, QtGui.QPalette.Text, SYS.QCl("#434343")
            )  ## <- Unused
            ...
            PLT_LIGHT.setColor(
                QtGui.QPalette.Disabled, QtGui.QPalette.Button, SYS.QCl("#252525")
            )  ## <- Unused
            ...
            PLT_DARK.setColor(
                QtGui.QPalette.Disabled, QtGui.QPalette.Text, SYS.QCl("#939393")
            )  ## <- Unused
...
class InputCore:
    ...
    def updateDetails(self, cmb=False):
        ...
        if UIZ == UIA:
            if HDB.CPLTR_SEARCH.completionCount() > 1 and not cmb:
                UIA.STATUSBAR.showMessage(
                    f"{HDB.CPLTR_SEARCH.completionCount()} suggestions"
                )  ## Sets number of suggestions to be displayed in status bar
            if len(UIA.LNE_SEARCH.text()) <= 0:
                UIA.STATUSBAR.showMessage(
                    ""
                )  ## TEMPORARY FIX for Status Bar Suggestions stucked

            if UIA.EXEC_POINTER == 0:
                UIA.CMB_RECENT.setCurrentIndex(
                    -1
                )  ## Deselects and clears current text from Recent List when user is using search box to reduce clutter and confusion.

        ENTRY = (
            UIZ.LNE_SEARCH.text() if not cmb else UIZ.CMB_RECENT.currentText()
        )  ## Substitutes the entry from Recent List when the pointer is set to 1(CMB)
        ENTRY_CMPLTN = HDB.CPLTR_SEARCH.currentCompletion()

        hN = ENTRY[:3]  ## Parse the entry by filtering only the Hymn Numbers

        if not hN.isdigit() or not int(
            hN
        ):  ## Prevents processing an incomplete user input
            if (
                ENTRY != "" and ENTRY_CMPLTN[:3] not in HYMNAL.MISSING.list
            ):  ## Instead of ignoring the whole entry,
                hN = ENTRY_CMPLTN[
                    :3
                ]  ## the current Completion will act as backup for entry
            else:  ## Otherwise, update the UI with respect to the empty entry.
                self.updateButtons(0)
                self.updatePreviews(0)
                UIZ.QUERY_TIMEOUT = (
                    False  ## Prevents dumping of query data when entry is invalid
                )
                return

        HINFO = HDB.getStats(hN)  ## Retrieve Hymn Info/Stats
        self.CURRENT_HINFO = (
            HINFO  ## Stores the Hymn Data as the program's current hymn.
        )
        ...
        if ENTRY != f"{HINFO.num} {HINFO.title}":
            if UIZ == UIA:
                UIZ.LBL_PVW_TITLE.setText(
                    HINFO.title
                )  ## Displays hymn title when current search text does not match the title anymore. This is to let the user know the title even when the search box is incomplete
            if UIZ == UIC:
                UIZ.LBL_PVW_TITLE.setText(
                    f'{HINFO.title} <font color="{QSS.TXT_DISABLED}">{f"({humanize.naturaltime(time.time()-HINFO.lastAccessed)})</font>" if HINFO.lastAccessed !=0 else ""}'
                )
        elif cmb and UIZ.CMB_RECENT.currentText() != f"{HINFO.num} {HINFO.title}":
            UIZ.LBL_PVW_TITLE.setText(
                HINFO.title
            )  ## Similar behavior from above but differs in source since the entry is from recent list.
        else:
            if UIZ != UIA:
                UIZ.LBL_PVW_TITLE.setText(
                    f'{HINFO.eqTitle} <font color="{QSS.TXT_DISABLED}">{f"({humanize.naturaltime(time.time()-HINFO.lastAccessed)})</font>" if HINFO.lastAccessed !=0 else ""}'
                )

        if UIZ == UIA:
            ## Equivalent Title
            UIA.LBL_PVW_EQUIV.setText(
                f"{HINFO.eqTitle} "
            )  ## Displays the equivalent Hymn title. This statement does not need a placeholder text since the variable inserted here has it incase of missing equivalents

            ## Last Accessed
            if HINFO.lastAccessed > 0:
                UIA.LBL_PVW_ACCSS.setText(
                    f"Last opened: {datetime.fromtimestamp(HINFO.lastAccessed).strftime('%b %d, %Y at %I:%M %p')} ({humanize.naturaltime(time.time()-HINFO.lastAccessed)})"
                )

            ## Launched (Status Bar)
            if HINFO.launches != 0 and UIA.LBL_PVW_ACCSS.text() != "":
                UIA.LBL_PVW_ACCSS.setStatusTip(
                    f'Launched {f"{HINFO.launches} times." if HINFO.launches > 1 else "once."}'
                )
    ...
    def updateButtons(self, mode):
        ...
        if mode == -1:  ## No Available File
            UIZ.BTN_LAUNCH.setEnabled(False)
            UIZ.BTN_LAUNCH.setText(BTN0[0])

        elif mode == 0:  ## Waiting
            UIZ.BTN_LAUNCH.setEnabled(False)
            UIZ.BTN_LAUNCH.setText(BTN0[1])

        elif mode == 1:  ## Ready
            UIZ.BTN_LAUNCH.setEnabled(True)
            UIZ.BTN_LAUNCH.setText(BTN0[2])

        elif mode == 2:  ## Launching
            UIZ.BTN_LAUNCH.setEnabled(False)
            UIZ.BTN_LAUNCH.setText(BTN0[3])
            keyboard.send(
                "esc"
            )  # Sends escape key to hide completer for search box if it's active

        elif mode == 3:  ## Launched
            UIZ.BTN_LAUNCH.setEnabled(False)
            UIZ.BTN_LAUNCH.setText(BTN0[4])
    ...
    def executeFile(self):
        ...
        for FILE in XDB.namelist():
            ...
            if self.CURRENT_HINFO.title in spH.title:
                ...
                if not KPath.exists(
                    FILEDIR
                ):  ## Only extract if the file is non-existent in temp folder in order to save time for extraction
                    XDB.extract(
                        FILE, SYS.DIR_TEMP
                    )  ## Extract to temporary directory set by System class
                    shutil.move(
                        f"{SYS.DIR_TEMP}\\{FILE}", SYS.DIR_TEMP
                    )  ## Relocate the file from a subfolder (EN/TL) to main temp directory
                break
        ...
        PROCESS = Thread(
            target=lambda: subprocess.Popen(
                f'cd {SYS.PPT_EXEC} & start POWERPNT.exe /{SW} "{FILEDIR}"', shell=True
            )
        )
        PROCESS.start()
        ...
        ARRAY = [
            self.CURRENT_HINFO.queries + 1,
            self.CURRENT_HINFO.launches + 1,
            time.time(),
        ]  ## Issue: Might need to build a separate function for this array incase the array is expanded in the future
        SDATA["DATA"].update({str(self.CURRENT_HINFO.num): ARRAY})
        SDB.dump()  ## Updates the main stat data (SDATA) and dumps into JSON using the Statistics Database class

        ## Update UI
        if UIZ == UIA:
            RECENT_ITEMS = [
                UIZ.CMB_RECENT.itemText(i)
                for i in range(int(CDATA["MaxAllowedRecent"]))
            ]
            UIZ.updateRecentList(
                1
                if f"{self.CURRENT_HINFO.num} {self.CURRENT_HINFO.title}"
                not in RECENT_ITEMS
                else -1
            )  ## Update Recent List so the recent launched file would show up
        self.updateDetails()
        ...
        if CDATA["KeepFocusOnBrowser"] != "True":
            time.sleep(
                0.3
            )  ## Places a 300ms delay for the program to simulate refocusing back to the program; This is also to minimize the possibility of glitches.
            if not UIB.isHidden():
                UIB.show()  ## Also shows the Settings window
            UIZ.setFocus(True)
            UIZ.activateWindow()
            UIZ.raise_()
            UIZ.show()  ## Refocuses the active window mode (Full or Compact mode)
            keyboard.send(
                "tab"
            )  ## Simulates tab keypress to automatically highlight the search bar which is on first index of the GUI layout

        ## Log
        PRS = Presentation(FILEDIR)
        SYS.CNT_SESSION_PRESN += 1
        LOG.info(
            f"Launched #{self.CURRENT_HINFO.num} {self.CURRENT_HINFO.title} ({len(PRS.slides)} Slide(s)) ({round((time.time()-EXEC_TIME)*1000)} ms) {getFileStat(FILEDIR, 'sz', szFmt='KB')} KB"
        )
    ...
    def searchFill(self, event):
        ...
        try:
            CHINFO = self.CURRENT_HINFO
        except AttributeError:
            CHINFO = HDB.getStats("001")
        NUM = int(CHINFO.num)

        DELTA = event.angleDelta().y()
        if DELTA == -120 and NUM <= SYS.HYMNS_MAX - 1:  ## Scroll Down
            UIZ.LNE_SEARCH.setText(
                f"{KString.toDigits(NUM+1, 3)} {HDB.getStats(KString.toDigits(NUM+1, 3)).title}"
            )
        elif DELTA == 120 and NUM >= 2:  ## Scroll Up
            if NUM > SYS.HYMNS_MAX:
                UIZ.LNE_SEARCH.setText(
                    f"{KString.toDigits(SYS.HYMNS_MAX, 3)} {HDB.getStats(KString.toDigits(SYS.HYMNS_MAX, 3)).title}"
                )
            else:
                UIZ.LNE_SEARCH.setText(
                    f"{KString.toDigits(NUM-1, 3)} {HDB.getStats(KString.toDigits(NUM-1, 3)).title}"
...
class FileManager:
    ...
    def deleteRecent(self, deleteAll=False):
        ...
        try:
            THRESHOLD = int(
                CDATA["MaxAllowedRecent"]
            )  ## Retrieves the amount of maximum allowed recent files
        except ValueError:
            THRESHOLD = SYS.RECENTS.DEFAULT
        if THRESHOLD < SYS.RECENTS.ALLOWEDMIN or THRESHOLD > SYS.RECENTS.ALLOWEDMAX:
            THRESHOLD = SYS.RECENTS.DEFAULT
    ...
    def getRecentFiles(self):
        return [
            f"{SYS.DIR_TEMP}\\{i}"
            for i in os.listdir(SYS.DIR_TEMP)
            if not i.startswith("~$") and i.endswith(".pptx")
        ]

    def getLogFiles(self):
        return [
            f"{SYS.DIR_LOG}\\{i}" for i in os.listdir(SYS.DIR_LOG) if i.endswith(".log")
        ]
    ...
    def deleteOldest(self, fileList, threshold, deleteAll):
        ...
        """
        while len(fileList()) > (
            threshold if not deleteAll else 0
        ):  ## Process code until the detected files are below threshold or when the switch is set to "Delete All" (0)
            try:
                os.remove(
                    min(fileList(), key=os.path.getatime)
                )  ## Eliminates the oldest accessed file
            except PermissionError as e:
                LOG.warn(
                    e
                )  ## Issue: There is no solution for this one yet. Administrator permissions could be used in future releases
            except FileNotFoundError as e:
                pass  ## Ignore when file is not there anymore
...
class ContextMenus:
    def __init__(self, UI):
        ...
        UI.ACT_MODE_SWITCH = UI.CTX_MENU.addAction(
            "Compact Mode" if UI == UIA else "Full Size Mode"
        )
        UI.ACT_MODE_SWITCH.setShortcut("Ctrl+M")
        UI.CTX_MENU.addSeparator()
        UI.ACT_MINIMIZE = UI.CTX_MENU.addAction("Minimize")
        UI.ACT_MINIMIZE.setShortcut(QtGui.QKeySequence("Alt+Down"))
        UI.CTX_MENU.addSeparator()
        UI.ACT_SETTINGS = UI.CTX_MENU.addAction("Settings")
        UI.ACT_SETTINGS.setShortcut(QtGui.QKeySequence("Alt+S"))
        UI.ACT_EXIT = UI.CTX_MENU.addAction("Exit")

        ## Connections
        QtWidgets.QShortcut(QtGui.QKeySequence("Ctrl+M"), UI).activated.connect(
            lambda: UI.switchMode()
        )
        QtWidgets.QShortcut(QtGui.QKeySequence("Alt+Down"), UI).activated.connect(
            lambda: UI.showMinimized()
        )
        QtWidgets.QShortcut(QtGui.QKeySequence("Alt+S"), UI).activated.connect(
            lambda: UIB.enterWindow()
        )
...
class Animations:
    ...
    class ButtonAnimation:
        def __init__(
            self, buttonObject, hexStart, hexEnd, textFg, textBg, duration=None
        ):
        ...
        def connectEvents(self):
            ...
            def animate(cycle):
                ...
                ) if cycle else self.ANIMATION.setDirection(
        ...
        def buttonValueChanged(self, color):
            ...
            self.FOREGROUND = (
                SYS.QCl(self.TEXT_FOREGROUND)
                if self.ANIMATION.direction() == QtCore.QAbstractAnimation.Forward
                else SYS.QCl(self.TEXT_BACKGROUND)
            )
...
class QWGT_BROWSER(QtWidgets.QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.EXEC_READY = (
            False  ## Set to True for when all is ready for launch. Otherwise, False
        )
    ...
    def setupUi(self):
        ...
        self.WIDGET.setObjectName("QWGT_BROWSER")
        self.LYT_GRID = QtWidgets.QGridLayout(self.WIDGET)
        self.LYT_GRID.setObjectName("LYT_GRID")
        self.GRID_HYMN = QtWidgets.QGridLayout()
        self.GRID_HYMN.setObjectName("GRID_HYMN")
        self.GRID_MAIN = QtWidgets.QGridLayout()
        self.GRID_MAIN.setObjectName("GRID_MAIN")

        SPC0 = QtWidgets.QSpacerItem(
            20, 40, QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Expanding
        )
        self.LYT_GRID.addItem(SPC0, 3, 1, 1, 1)
        SPC1 = QtWidgets.QSpacerItem(
            40, 20, QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Minimum
        )
        self.LYT_GRID.addItem(SPC1, 2, 2, 1, 1)
        SPC2 = QtWidgets.QSpacerItem(
            40, 20, QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Minimum
        )
        self.GRID_MAIN.addItem(SPC2, 4, 0, 1, 1)
        SPC3 = QtWidgets.QSpacerItem(
            40, 20, QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Minimum
        )
        self.LYT_GRID.addItem(SPC3, 2, 0, 1, 1)
        SPC4 = QtWidgets.QSpacerItem(
            20, 40, QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Expanding
        )
        self.LYT_GRID.addItem(SPC4, 1, 1, 1, 1)

        self.LBL_BROWSER = QtWidgets.QLabel(self.WIDGET)
        self.LBL_BROWSER.setObjectName("LBL_BROWSER")
        # self.LBL_BROWSER.setAlignment(QtCore.Qt.AlignCenter)
        self.LBL_BROWSERB = QtWidgets.QLabel(self.WIDGET)
        self.LBL_BROWSERB.setObjectName("LBL_BROWSERB")
        # self.LBL_BROWSER.setAlignment(QtCore.Qt.AlignCenter)
        self.LNE_SEARCH = QtWidgets.QLineEdit(self.WIDGET)
        self.LNE_SEARCH.setObjectName("LNE_SEARCH")
        self.LNE_SEARCH.setMinimumSize(300, 0)
        self.LNE_SEARCH.setClearButtonEnabled(True)
        self.BTN_ADDQUEUE = QtWidgets.QPushButton(self.WIDGET)
        self.BTN_ADDQUEUE.setObjectName("BTN_ADDQUEUE")
        self.BTN_QUEUES = QtWidgets.QPushButton(self.WIDGET)
        self.BTN_QUEUES.setObjectName("BTN_QUEUES")
        self.BTN_MORE = QtWidgets.QPushButton(self.WIDGET)
        self.BTN_MORE.setObjectName("BTN_MORE")
        self.LBL_PVW_TITLE = QtWidgets.QLabel(self.WIDGET)
        self.LBL_PVW_TITLE.setObjectName("LBL_PVW_TITLE")
        self.LBL_PVW_EQUIV = QtWidgets.QLabel(self.WIDGET)
        self.LBL_PVW_EQUIV.setObjectName("LBL_PVW_EQUIV")
        self.LBL_PVW_ACCSS = QtWidgets.QLabel(self.WIDGET)
        self.LBL_PVW_ACCSS.setObjectName("LBL_PVW_ACCSS")
        self.LBL_PVW_ACCSS.setEnabled(False)
        self.CMB_RECENT = QtWidgets.QComboBox(self.WIDGET)
        self.CMB_RECENT.setObjectName("CMB_RECENT")
        self.CMB_RECENT.setEditable(True)
        self.CMB_RECENT.setMinimumWidth(300)
        self.CMB_RECENT.lineEdit().setReadOnly(1)
        self.CMB_RECENT.view().window().setWindowFlags(
            Qt.Popup | Qt.FramelessWindowHint
        )
        self.CMB_RECENT.view().window().setAttribute(Qt.WA_TranslucentBackground)
        self.BTN_LAUNCH = QtWidgets.QPushButton(self.WIDGET)
        self.BTN_LAUNCH.setObjectName("BTN_LAUNCH")
        self.BTN_LAUNCH.setEnabled(False)

        ACT_RET = QtWidgets.QAction(self, triggered=self.BTN_LAUNCH.animateClick)
        ACT_RET.setShortcut(QtGui.QKeySequence("Return"))
        ACT_ENTER = QtWidgets.QAction(self, triggered=self.BTN_LAUNCH.animateClick)
        ACT_ENTER.setShortcut(QtGui.QKeySequence("Enter"))
        ...
        TITLE_FONT = QFont("Integral CF", 14)
        ...
        self.CMB_RECENT.currentIndexChanged.connect(
            lambda: self.INP.updateDetails(True)
        )
    ...
    def switchMode(self):
        ...
        DELTA = UIA.geometry()
        UIC.move(
            DELTA.x() + 100, DELTA.y() + 100
        )  ## Calculate the change in position so the next window would appear near it.
        CDATA["CompactMode"] = "True"
        CFG.dump()
        global UIZ
        UIZ = UIC
    ...
    def updateRecentList(self, whenLaunched=0):
        ...
        FILES = [
            f"{SYS.DIR_TEMP}\\{x}"
            for x in os.listdir(SYS.DIR_TEMP)
            if not x.startswith("~$") and x.endswith(".pptx")
        ]

        ## Sort the files based on their accessed time state
        SORTED = [
            f"{x[len(SYS.DIR_TEMP)+1:-5]}"
            for x in sorted(FILES, key=os.path.getatime, reverse=True)
        ]
        ...
        if whenLaunched:
            SORTED[
                SORTED.index(
                    f"{self.INP.CURRENT_HINFO.num} {self.INP.CURRENT_HINFO.title}"
                )
            ] = "0"
            ...
            for i in reversed(range(int(CDATA["MaxAllowedRecent"]))):
                if i == SORTED.index(
                    "0"
                ):  ## When the index reaches the index where we should do the adjustment
                    for j in range(i):
                        SORTED[i - (j)] = SORTED[
                            i - (j + 1)
                        ]  ## Move all remaining indexes (decreasing) by one step
                    SORTED[
                        0
                    ] = f"{self.INP.CURRENT_HINFO.num} {self.INP.CURRENT_HINFO.title}"  ## Sets the recent launched file to index zero[0] so it would show up first
                    break

        ## Insert all sorted files into combo box
        self.CMB_RECENT.clear()
        for i in range(len(SORTED)):
            self.CMB_RECENT.addItem(SORTED[i])
...
class QWGT_SETTINGS(QtWidgets.QMainWindow):
    def __init__(self, parent=None):
        self.HASCHANGES = (
            0  # Indicating that the settings window is in it's unchanged state
        )
    ...
    def setupUi(self):
        ...
        self.WIDGET = QtWidgets.QWidget(self)
        self.WIDGET.setObjectName("QWGT_SETTINGS")

        self.LYT_GRID_SETTINGS = QtWidgets.QGridLayout(self.WIDGET)
        self.LYT_GRID_SETTINGS.setObjectName("LYT_GRID_SETTINGS")
        self.FORM_SETTINGS = QtWidgets.QGridLayout()
        self.FORM_SETTINGS.setObjectName("FORM_SETTINGS")
        spacerItem2 = QtWidgets.QSpacerItem(
            40, 20, QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Minimum
        )
        self.LST_NORTH = QtWidgets.QListWidget(self.WIDGET)
        self.LST_NORTH.setObjectName("LST_NORTH")
        self.LST_NORTH.setFixedSize(QtCore.QSize(150, 600))
        self.LST_SOUTH = QtWidgets.QListWidget(self.WIDGET)
        self.LST_SOUTH.setObjectName("LST_SOUTH")
        self.LST_SOUTH.setFixedSize(QtCore.QSize(150, 50))
        self.LST_PANELS = [self.LST_NORTH, self.LST_SOUTH]

        self.GRID_FOOTER = QtWidgets.QGridLayout()
        self.GRID_FOOTER.setObjectName("GRID_FOOTER")

        self.STACKED_ACS_NORTH = QtWidgets.QStackedWidget(self.WIDGET)
        self.STACKED_ACS_NORTH.setObjectName("STACKED_ACS_NORTH")
        self.STACKED_ACS_SOUTH = QtWidgets.QStackedWidget(self.WIDGET)
        self.STACKED_ACS_SOUTH.setObjectName("STACKED_ACS_SOUTH")
        ...
        for i in range(2):
            self.FORM_SETTINGS.addWidget(self.STACKED_ACCESS[i], 0, 1, 2, 1)
        ...
        ## Dock Settings
        self.TBL_DOCK = QtWidgets.QDockWidget(UIB)
        self.TBL_DOCK.setObjectName("TBL_DOCK")
        self.TBL_DOCK.setFeatures(
            QtWidgets.QDockWidget.DockWidgetFloatable
            | QtWidgets.QDockWidget.DockWidgetMovable
        )
        self.TBL_DOCK.setAllowedAreas(Qt.RightDockWidgetArea)
        self.TBL_DOCK_CONTENTS = QtWidgets.QWidget()
        self.TBL_DOCK.setObjectName("TBL_DOCK_CONTENTS")
        self.TBL_GRID = QtWidgets.QGridLayout(self.TBL_DOCK_CONTENTS)
        self.TBL_GRID.setObjectName("TBL_GRID")
        self.TBL_STATISTICS = QtWidgets.QTableWidget(self.TBL_DOCK_CONTENTS)
        self.TBL_STATISTICS.setObjectName("TBL_STATISTICS")
        self.TBL_STATISTICS.setSizeAdjustPolicy(
            QtWidgets.QAbstractScrollArea.AdjustToContents
        )
        self.TBL_STATISTICS_LOADED = False
        self.BTN_REFRESH_STATS = QtWidgets.QPushButton(self.TBL_DOCK_CONTENTS)
        self.BTN_REFRESH_STATS.setObjectName("BTN_REFRESH_STATS")
        self.BTN_DOCK_EXPORTCSV = QtWidgets.QPushButton(self.TBL_DOCK_CONTENTS)
        self.BTN_DOCK_EXPORTCSV.setObjectName("BTN_DOCK_EXPORTCSV")
        SPCH = QtWidgets.QSpacerItem(
            40, 20, QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Minimum
        )
        ...
        self.BTN_REFRESH_STATS.clicked.connect(
            lambda: self.CLASS_STS.forceRefreshStats()
        )
        self.BTN_DOCK_EXPORTCSV.clicked.connect(
            lambda: self.CLASS_STS.exportStatsTable()
        )
    ...
    def generatePanels(self):
        ...
        self.DLG_LST_PANELS = QtWidgets.QStyledItemDelegate()
        self.DLG_LST_PANELS.setObjectName("DLG_LST_PANELS")
    ...
    def enterWindow(self):
        if UIB.isHidden():
            ...
            self.updateSettingItems(
                0
            )  ## General Tab should be the default highlighted panel

        UIB.activateWindow()
        UIB.show()
        UIB.raise_()
        UIB.setWindowState(
            UIB.windowState() & ~QtCore.Qt.WindowMinimized | QtCore.Qt.WindowActive
        )
    ...
    def triggerButton(self, mode):
        ...
        if not mode:
            if self.ACTIVE_PAGE != (0, 1):
                ...
                MSG_BOX.setText(
                    "This will erase all saved preferences.\nDo you want to continue?"
                )
                ...
                if RET == QtWidgets.QMessageBox.Yes:
                    ...
                    SYS.CURR_THEME = (
                        int(CDATA["Theme"]) if int(CDATA["Theme"]) in [0, 1] else 1
                    )
                ...
                MSG_BOX.setIcon(QtWidgets.QMessageBox.Warning)
                MSG_BOX.setText(
                    "This will permanently erase all saved data for hymnal.\nDo you want to continue?"
                )
    ...
    def updateSettingItems(self, idx):
        ...
        self.STACKED_ACCESS[invert(idx)].hide()
        self.LST_PANELS[invert(idx)].clearSelection()
        ...
        if not idx:
            ...
            if ID == 1:  ## Hymnal Page
                self.TBL_DOCK.show()
                self.BTN_RESET.show()
                UIB.BTN_RESET.setText("Reset Stats")
    ...
    class General:
        def __init__(self):
            self.PAGE_AC_GEN = QtWidgets.QWidget()
            self.PAGE_AC_GEN.setObjectName("PAGE_AC_GEN")

            self.LBL_DARKMODE = QtWidgets.QLabel(self.PAGE_AC_GEN)
            self.LBL_DARKMODE.setObjectName("LBL_DARKMODE")
            self.CBX_DARKMODE = QtWidgets.QComboBox(self.PAGE_AC_GEN)
            self.CBX_DARKMODE.setObjectName("CBX_DARKMODE")
            self.CBX_DARKMODE.setEditable(0)
            self.CBX_DARKMODE.view().window().setWindowFlags(
                Qt.Popup | Qt.FramelessWindowHint
            )
            self.CBX_DARKMODE.view().window().setAttribute(Qt.WA_TranslucentBackground)
            self.CHK_ALWAYSONTOP = QtWidgets.QCheckBox(self.PAGE_AC_GEN)
            self.CHK_ALWAYSONTOP.setObjectName("CHK_ALWAYSONTOP")
            self.LBL_WINOPACITY = QtWidgets.QLabel(self.PAGE_AC_GEN)
            self.LBL_WINOPACITY.setObjectName("LBL_WINOPACITY")
            self.SLD_WINOPACITY = QtWidgets.QSlider(Qt.Horizontal, self.PAGE_AC_GEN)
            self.SLD_WINOPACITY.setObjectName("SLD_WINOPACITY")
            self.SLD_WINOPACITY.setMaximumWidth(70)
            self.SLD_WINOPACITY.setMinimum(50)
            self.SLD_WINOPACITY.setMaximum(100)

            self.CHK_AUTOSLIDESHOW = QtWidgets.QCheckBox(self.PAGE_AC_GEN)
            self.CHK_AUTOSLIDESHOW.setObjectName("CHK_AUTOSLIDESHOW")
            self.CHK_KEEP_FOCUS_WHEN_LAUNCH = QtWidgets.QCheckBox(self.PAGE_AC_GEN)
            self.CHK_KEEP_FOCUS_WHEN_LAUNCH.setObjectName("CHK_KEEP_FOCUS_WHEN_LAUNCH")

            self.GBX_INTERFACE = QtWidgets.QGroupBox("Interface", self.PAGE_AC_GEN)
            self.GBX_INTERFACE.setObjectName("GBX_INTERFACE")
            self.GBX_EXECUTION = QtWidgets.QGroupBox("Execution", self.PAGE_AC_GEN)
            self.GBX_EXECUTION.setObjectName("GBX_EXECUTION")
            self.LYT_INTERFACE = QtWidgets.QGridLayout(self.GBX_INTERFACE)
            self.LYT_INTERFACE.setObjectName("LYT_INTERFACE")
            self.LYT_EXECUTION = QtWidgets.QGridLayout(self.GBX_EXECUTION)
            self.LYT_EXECUTION.setObjectName("LYT_EXECUTION")

            SPCV = QtWidgets.QSpacerItem(
                20, 40, QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Expanding
            )
            SPCH = QtWidgets.QSpacerItem(
                40, 20, QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Minimum
            )
            self.GRID_AC_GEN = QtWidgets.QGridLayout(self.PAGE_AC_GEN)
            self.GRID_AC_GEN.setObjectName("GRID_AC_GEN")
            self.GRID_AC_GEN.addItem(
                SPCV, 50, 0, 1, 1
            )  # Vertical Spacer inside settings
            self.GRID_AC_GEN.addItem(
                SPCH, 1, 50, 1, 1
            )  # Horizontal Spacer inside settings
            ...
            ## Configurations
            self.CHK_ALWAYSONTOP.setChecked(
                True if CDATA["AlwaysOnTop"] == "True" else False
            )
            self.CHK_AUTOSLIDESHOW.setChecked(
                True if CDATA["AutoSlideshow"] == "True" else False
            )
            self.SLD_WINOPACITY.setValue(int(CDATA["WindowOpacity"]))
            self.SLD_WINOPACITY.setTickPosition(QtWidgets.QSlider.TicksAbove)
            if self.SLD_WINOPACITY.value() < SYS.MIN_OPACITY:
                self.SLD_WINOPACITY.setValue(SYS.MIN_OPACITY)  ## Floor
            UIA.setWindowOpacity(self.SLD_WINOPACITY.value() / 100)
            UIC.setWindowOpacity(self.SLD_WINOPACITY.value() / 100)
            for name in ["Light", "Dark", "Black"]:
                self.CBX_DARKMODE.addItem(name)
            self.CBX_DARKMODE.setCurrentIndex(SYS.CURR_THEME)

            ## Connections
            self.CBX_DARKMODE.currentIndexChanged.connect(
                lambda: self.updateThemeSelect()
            )
            self.CHK_ALWAYSONTOP.stateChanged.connect(lambda: self.updateCheckboxes(1))
            self.CHK_AUTOSLIDESHOW.stateChanged.connect(
                lambda: self.updateCheckboxes(2)
            )
            self.CHK_KEEP_FOCUS_WHEN_LAUNCH.stateChanged.connect(
                lambda: self.updateCheckboxes(3)
            )
            ...
            self.CHK_KEEP_FOCUS_WHEN_LAUNCH.setText(
                "Keep focus on browser when opening a file"
            )
        ...
        def updateThemeSelect(self):
            ...
            CDATA["Theme"] = str(SYS.CURR_THEME)
            CFG.dump()
            QSS.initStylesheet()
        ...
        def updateWindowOpacity(self):
            ...
            UIA.setWindowOpacity((self.SLD_WINOPACITY.value()) / 100)
            UIC.setWindowOpacity((self.SLD_WINOPACITY.value()) / 100)
            CDATA["WindowOpacity"] = str(self.SLD_WINOPACITY.value())
            CFG.dump()

        def updateCheckboxes(self, mode):
            if mode == 1:
                ## Always on top
                CDATA["AlwaysOnTop"] = str(invert(CDATA["AlwaysOnTop"], True))
                CFG.dump()
                if CDATA["AlwaysOnTop"] == "True":
                    UIA.setWindowFlags(
                        UIA.windowFlags() | QtCore.Qt.WindowStaysOnTopHint
                    )
                    UIB.setWindowFlags(
                        UIB.windowFlags() | QtCore.Qt.WindowStaysOnTopHint
                    )
                    UIC.setWindowFlags(
                        UIC.windowFlags() | QtCore.Qt.WindowStaysOnTopHint
                    )
                else:
                    UIA.setWindowFlags(
                        UIA.windowFlags() & ~QtCore.Qt.WindowStaysOnTopHint
            ...
            if mode == 2:
                # Auto Slideshow
                CDATA["AutoSlideshow"] = (
                    "False" if CDATA["AutoSlideshow"] == "True" else "True"
                )
                CFG.dump()

            if mode == 3:
                # Auto Slideshow
                CDATA["KeepFocusOnBrowser"] = (
                    "False" if CDATA["KeepFocusOnBrowser"] == "True" else "True"
                )
                CFG.dump()

    class Statistics:
        def __init__(self):
            self.PAGE_AC_STS = QtWidgets.QWidget()
            self.PAGE_AC_STS.setObjectName("PAGE_AC_STS")

            self.GBX_DATABASE = QtWidgets.QGroupBox("Database", self.PAGE_AC_STS)
            self.GBX_DATABASE.setObjectName("GBX_DATABASE")
            self.GBX_FILES = QtWidgets.QGroupBox("Recent Files", self.PAGE_AC_STS)
            self.GBX_FILES.setObjectName("GBX_FILES")
            self.GBX_STATISTICS = QtWidgets.QGroupBox("Statistics", self.PAGE_AC_STS)
            self.GBX_STATISTICS.setObjectName("GBX_STATISTICS")
            self.LYT_DATABASE = QtWidgets.QGridLayout(self.GBX_DATABASE)
            self.LYT_DATABASE.setObjectName("LYT_DATABASE")
            self.LYT_FILES = QtWidgets.QGridLayout(self.GBX_FILES)
            self.LYT_FILES.setObjectName("LYT_FILES")
            self.LYT_STATISTICS = QtWidgets.QGridLayout(self.GBX_STATISTICS)
            self.LYT_STATISTICS.setObjectName("LYT_STATISTICS")

            self.LBL_DBSTATUS = QtWidgets.QLabel(self.PAGE_AC_STS)
            self.LBL_DBSTATUS.setObjectName("LBL_DBSTATUS")
            self.LBL_STATS = QtWidgets.QLabel(self.PAGE_AC_STS)
            self.LBL_STATS.setObjectName("LBL_STATS")

            self.LBL_RECENT_DETAILS = QtWidgets.QLabel(self.PAGE_AC_STS)
            self.LBL_RECENT_DETAILS.setObjectName("LBL_RECENT_DETAILS")
            self.LBL_MAX_ALLOWED_RECENT = QtWidgets.QLabel(self.PAGE_AC_STS)
            self.LBL_MAX_ALLOWED_RECENT.setObjectName("LBL_MAX_ALLOWED_RECENT")
            self.SBX_MAX_ALLOWED_RECENT = QtWidgets.QSpinBox(self.GBX_FILES)
            self.SBX_MAX_ALLOWED_RECENT.setObjectName("SBX_MAX_ALLOWED_RECENT")
            self.SBX_MAX_ALLOWED_RECENT.setMaximum(
                SYS.RECENTS.ALLOWEDMAX
            ), self.SBX_MAX_ALLOWED_RECENT.setMinimum(SYS.RECENTS.ALLOWEDMIN)
            self.SBX_MAX_ALLOWED_RECENT.wheelEvent = (
                lambda event: None
            )  ## Prevents scroll
            self.SBX_MAX_ALLOWED_RECENT.lineEdit().setContentsMargins(20, 2, 2, 2)
            self.BTN_CLEAR_RECENT = QtWidgets.QPushButton(self.GBX_FILES)
            self.BTN_CLEAR_RECENT.setObjectName("BTN_CLEAR_RECENT")
            self.BTN_CLEAR_RECENT.setMaximumWidth(120)

            self.BTN_TARGET_PATH = QtWidgets.QPushButton(self.PAGE_AC_STS)
            self.BTN_TARGET_PATH.setObjectName("BTN_TARGET_PATH")
            self.LNE_TARGET_PATH = QtWidgets.QLineEdit(self.PAGE_AC_STS)
            self.LNE_TARGET_PATH.setObjectName("LNE_TARGET_PATH")
            self.LNE_TARGET_PATH.setReadOnly(1)

            self.LBL_STATISTICS = QtWidgets.QLabel(self.PAGE_AC_STS)
            self.LBL_STATISTICS.setObjectName("LBL_STATISTICS")
            self.BTN_IMPORT = QtWidgets.QPushButton(self.PAGE_AC_STS)
            self.BTN_IMPORT.setObjectName("BTN_IMPORT")
            self.BTN_EXPORT = QtWidgets.QPushButton(self.PAGE_AC_STS)
            ...
            self.GRID_AC_STS = QtWidgets.QGridLayout(self.PAGE_AC_STS)
            self.GRID_AC_STS.setObjectName("GRID_AC_STS")
            ...
            ## Connections
            self.SBX_MAX_ALLOWED_RECENT.valueChanged.connect(
                lambda: self.updateRecentFiles(0)
            )
            ...
            self.SBX_MAX_ALLOWED_RECENT.setValue(
                int(CDATA["MaxAllowedRecent"])
                if CDATA["MaxAllowedRecent"].isdigit()
                else SYS.RECENTS.DEFAULT
            )
        ...
        def changeDatabase(self):
            PKG_PATH = QtWidgets.QFileDialog.getOpenFileName(
                None, "Browse for Hymnal Package", os.getcwd(), "SDA Package (*.sda)"
            )
            if PKG_PATH[0] != "":
        ...
        def updateRecentDetails(self):
            ...
            """
            PATHS = FMN.getRecentFiles()
            TOTAL = len(PATHS)
            SIZES = [getFileStat(PATH, "sz", szFmt="MB", szRnd=2) for PATH in PATHS]
            self.LBL_RECENT_DETAILS.setText(
                f'{f"{TOTAL} File(s) " if TOTAL !=0 else ""} <font color={str(QSS.TXT_DISABLED)}>{f"({round(sum(SIZES), 2)} MB)" if TOTAL !=0 else "No Files"}</font>'
            )
        ...
        def updateRecentFiles(self, mode):
            ...
            if not mode:
                ...
                CDATA["MaxAllowedRecent"] = str(VAL)
                CFG.dump()
                FMN.deleteRecent()
            elif mode:
        ...
        def updateWidget(self):
            ...
            if HYMNAL.MISSING.length:
                SIDENOTE = f"(There are {HYMNAL.MISSING.length} missing hymns.)"
            self.LBL_STATS.setWordWrap(True)
            self.LBL_DBSTATUS.setWordWrap(True)
            self.LBL_DBSTATUS.setText(
                f"<strong>{HYMNAL.TOTAL.ALL} / {SYS.HYMNS_MAX}</strong> {SIDENOTE} <font color='{QSS.TXT_DISABLED}'>({round(DBSIZE.val,2)} {DBSIZE.unit})</font>"
            )
            self.LBL_STATS.setText(
                f"{STATS.queries:,.0f} Total Queries\n{STATS.launches:,.0f} Total Launches"
            )
        ...
        def updateResizing(self):
            ...
            if UIB.TBL_DOCK.isFloating():
                ...
                if UIB.LST_PANELS[0].currentRow() == 1:
                    UIB.setFixedSize(QtCore.QSize(800, 480))
        ...
        def updateStatsTable(self, isNew=False):
            ...
            for i in range(len(TSL)):
                UIB.TBL_STATISTICS.setHorizontalHeaderItem(
            ...
            ## ROW
            for i in range(ROWS):
                UIB.TBL_STATISTICS.setVerticalHeaderItem(
                    i, QtWidgets.QTableWidgetItem()
                )
                UIB.TBL_STATISTICS.verticalHeaderItem(i).setText(str(i + 1))
                UIB.TBL_STATISTICS.verticalHeaderItem(i).setTextAlignment(
                    Qt.AlignHCenter
                )
                ...
                for j in range(COLS):
                    ...
                    UIB.TBL_STATISTICS.item(i, j).setFlags(
                ...
                try:
                    ...
                    h[4].setText(
                        HYMN.lastAcssHumanize if HYMN.lastAccessed != 0 else ""
                    )
        ...
        def exportStatsTable(self):
            ...
            with open(
                f'HDB Stats {datetime.now().strftime("%m-%d-%Y")}.csv', "w"
            ) as stream:
    ...
    class Library:
        def __init__(self):
            self.PAGE_AC_LIB = QtWidgets.QWidget()
            self.PAGE_AC_LIB.setObjectName("PAGE_AC_LIB")
            self.LBL_STATUS = QtWidgets.QLabel(self.PAGE_AC_LIB)
            self.LBL_STATUS.setObjectName("LBL_FOOTER_VER")
            self.LBL_STATUS.setEnabled(0)
            self.GRID_AC_FAV = QtWidgets.QGridLayout(self.PAGE_AC_LIB)
            self.GRID_AC_FAV.setObjectName("GRID_AC_FAV")
    ...
    class ShowLog:
        def __init__(self):
            self.PAGE_AC_LOG = QtWidgets.QWidget()
            self.PAGE_AC_LOG.setObjectName("PAGE_AC_LIB")
            self.CBX_LOG_LIST = QtWidgets.QComboBox(self.PAGE_AC_LOG)
            self.CBX_LOG_LIST.setObjectName("CBX_LOG_LIST")
            self.CBX_LOG_LIST.setEditable(0)
            self.CBX_LOG_LIST.view().window().setWindowFlags(
                Qt.Popup | Qt.FramelessWindowHint
            )
            self.CBX_LOG_LIST.view().window().setAttribute(Qt.WA_TranslucentBackground)
            self.CHK_AUTO_SCROLL = QtWidgets.QCheckBox(self.PAGE_AC_LOG)
            self.CHK_AUTO_SCROLL.setObjectName("CHK_AUTO_SCROLL")
            self.CHK_AUTO_SCROLL.setText("Auto Scroll")
            self.LBL_LOG_CONTENT = QtWidgets.QLabel(self.PAGE_AC_LOG)
            self.LBL_LOG_CONTENT.setObjectName("LBL_LOG_CONTENT")
            self.LBL_LOG_CONTENT.setEnabled(0)
            self.PTE_LOG_CONTENT = QtWidgets.QPlainTextEdit(self.PAGE_AC_LOG)
            self.PTE_LOG_CONTENT.setObjectName("PTE_LOG_CONTENT")
            ...
            self.GRID_AC_LOG = QtWidgets.QGridLayout(self.PAGE_AC_LOG)
            self.GRID_AC_LOG.setObjectName("GRID_AC_FAV")
            SPCH = QtWidgets.QSpacerItem(
                40, 20, QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Minimum
            )
            ...
            ## Load Config
            self.CHK_AUTO_SCROLL.setCheckState(
                0 if CDATA["AutoScroll"] == "True" else 2
            )

            ## Generate Log File Items
            self.LOG_LIST = sorted(
                [
                    (int(getFilename(filepath, "\\").split()[1][:-4]), filepath)
                    for filepath in FMN.getLogFiles()
                ],
                reverse=True,
            )
            for i, tms in enumerate([i for i, j in self.LOG_LIST]):
                dtTms = datetime.fromtimestamp(tms).strftime(
                    "%I:%M %p %x"
                ), datetime.fromtimestamp(tms).strftime("%I:%M %p")
                if not i:
                    self.CBX_LOG_LIST.addItem(f"Log #{i+1}: Current File")
                else:
                    self.CBX_LOG_LIST.addItem(
                        f"Log #{i+1}: {humanize.naturaltime(time.time()-tms) if time.time()-tms < 86400 else dtTms[0]}"
                    )

            self.CBX_LOG_LIST.currentIndexChanged.connect(
                lambda: self.updateLogContents()
            )
            ...
            self.V_SCROLLBAR, self.H_SCROLLBAR = (
                self.PTE_LOG_CONTENT.verticalScrollBar(),
                self.PTE_LOG_CONTENT.horizontalScrollBar(),
            )
        ...
        def updateAutoScroll(self):
            STATE = (
                (0, "False") if not self.CHK_AUTO_SCROLL.isChecked() else (2, "True")
            )
            CDATA["AutoScroll"] = STATE[1]
            CFG.dump()
            self.CHK_AUTO_SCROLL.setCheckState(STATE[0])

        def updateLogContents(self):
            self.LBL_LOG_CONTENT.setText(
                f'Log File | {datetime.now().strftime("%I:%M:%S %p")} (Usage: {round((SYS.PROCESS.memory_info().rss)/10**6, 2)} MB)'
            )

            if (
                not self.PTE_LOG_CONTENT.hasFocus()
            ):  ## Prevents auto-refresh and unintentional deselect when the cursor is focused.
                try:
                    with open(
                        self.LOG_LIST[self.CBX_LOG_LIST.currentIndex()][1]
                    ) as FILE:
                        CONTENT = FILE.read()
                except FileNotFoundError as e:
                    CONTENT = "Log unavailable. The file might be already removed."
                ...
                if self.CHK_AUTO_SCROLL.isChecked():
                    self.PTE_LOG_CONTENT.verticalScrollBar().setValue(
                        self.PTE_LOG_CONTENT.verticalScrollBar().maximum()
                    )  ## Keep the log scrolled to bottom
                self.LOCKED = False

    class About:
        def __init__(self):
            self.PAGE_AC_ABT = QtWidgets.QWidget()
            self.PAGE_AC_ABT.setObjectName("PAGE_AC_ABT")

            self.LBL_FOOTER = QtWidgets.QLabel(self.PAGE_AC_ABT)
            self.LBL_FOOTER.setObjectName("LBL_FOOTER")
            self.LBL_FOOTER_NOTE = QtWidgets.QLabel(self.PAGE_AC_ABT)
            self.LBL_FOOTER_NOTE.setObjectName("LBL_FOOTER_NOTE")
            self.LBL_FOOTER_NOTE.setEnabled(1)
            self.LBL_FOOTER_VER = QtWidgets.QLabel(self.PAGE_AC_ABT)
            self.LBL_FOOTER_VER.setObjectName("LBL_FOOTER_VER")
            self.LBL_FOOTER_VER.setEnabled(0)
            self.LBL_LOGO = QtWidgets.QLabel(self.PAGE_AC_ABT)
            self.LBL_FOOTER.setObjectName("LBL_LOGO")
            self.BTN_REPORT_FEEDBACK = QtWidgets.QPushButton(self.PAGE_AC_ABT)
            self.BTN_REPORT_FEEDBACK.setObjectName("BTN_REPORT_FEEDBACK")
            self.BTN_REPORT_FEEDBACK.setMaximumWidth(120)
            self.BTN_REPORT_FEEDBACK.setText("Report a feedback")

            self.LBL_LOGO.setPixmap(
                QtGui.QPixmap(SYS.RES_LOGO).scaledToHeight(150, Qt.SmoothTransformation)
            )

            SPCV = QtWidgets.QSpacerItem(
                20, 40, QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Expanding
            )
            SPCV2 = QtWidgets.QSpacerItem(
                20, 40, QtWidgets.QSizePolicy.Minimum, QtWidgets.QSizePolicy.Expanding
            )
            SPCH = QtWidgets.QSpacerItem(
            ...
            self.GRID_AC_ABT = QtWidgets.QGridLayout(self.PAGE_AC_ABT)
            self.GRID_AC_ABT.setObjectName("GRID_AC_ABT")
            self.GRID_AC_ABT.addItem(
                SPCH, 1, 50, 1, 1
            )  # Horizontal Spacer inside settings
            ...
            self.LBL_FOOTER.setTextInteractionFlags(
                self.LBL_FOOTER.textInteractionFlags() | Qt.LinksAccessibleByMouse
            )
            self.LBL_FOOTER_NOTE.setOpenExternalLinks(True)
            self.LBL_FOOTER_NOTE.setTextInteractionFlags(
                self.LBL_FOOTER_NOTE.textInteractionFlags() | Qt.LinksAccessibleByMouse
            )

        def displayText(self):
            self.LBL_FOOTER.setText(
                f'<font>{SW.PARENT_NAME} {SW.NAME}<br><br>A general hymnal browser for Seventh-day Adventist Church that supports up to {SYS.HYMNS_MAX} Hymns based on SDA Hymnal Philippine Edition.<br><br>This software is part of MSDAC System\'s collection of softwares<br>© {SW.PROD_YEAR} <a href="https://m.me/verdaderoken">Ken Verdadero</a> <a href="https://m.me/reynald.ycong">Reynald Ycong</a><br><br><a href="hhttps://github.com/msdacsystems/hymnalbrowser">GitHub Repository</a><br><a href="https://discord.gg/Ymsa2BUhJp">Discord Support Server</a></font>'
            )
            self.LBL_FOOTER_NOTE.setText(
                f"<font size=-1 color={QSS.TXT_DISABLED}>This program is still under development. In case of any unexpected bugs and crashes, we encourage you to report the problem on our Discord support server or send us a feedback using the feedback button.<br><br>The Seventh-day Adventist logo is a trademark property of General Conference of Seventh-day Adventists®.</font>"
            )
            self.LBL_FOOTER_VER.setText(
                f"<font size=-1>Made for Seventh-day Adventist Church<br>© MSDAC Systems {SW.PROD_YEAR} v{SW.VERSION} {SW.VERSION_NAME}</font>"
            )
...
class QWGT_REPORT_FEEDBACK(QtWidgets.QMainWindow):
    ...
    def setupUi(self):
        ...
        if CDATA["AlwaysOnTop"] == "True":
            self.setWindowFlags(self.windowFlags() | QtCore.Qt.WindowStaysOnTopHint)
        self.WGT_CENTRAL = QtWidgets.QWidget(self)
        self.WGT_CENTRAL.setObjectName("WGT_CENTRAL")
        self.LYT_MAIN = QtWidgets.QGridLayout(self.WGT_CENTRAL)
        self.LYT_MAIN.setObjectName("gridLayout_2")
        self.GRID_BODY = QtWidgets.QGridLayout()
        self.GRID_BODY.setObjectName("GRID_BODY")
        self.LBL_INSTRUCTION = QtWidgets.QLabel(self.WGT_CENTRAL)
        self.LBL_INSTRUCTION.setObjectName("LBL_INSTRUCTION")
        self.LBL_CHAR_INDIC = QtWidgets.QLabel(self.WGT_CENTRAL)
        self.LBL_CHAR_INDIC.setObjectName("LBL_CHAR_INDIC")
        self.LBL_CHAR_INDIC.setAlignment(Qt.AlignRight)
        self.PTE_FEEDBACK = QtWidgets.QPlainTextEdit(self.WGT_CENTRAL)
        self.PTE_FEEDBACK.setObjectName("PTE_FEEDBACK")
        self.LBL_AGREEMENT = QtWidgets.QLabel(self.WGT_CENTRAL)
        self.LBL_AGREEMENT.setObjectName(
            "LBL_AGREEMENT"
        ), self.LBL_AGREEMENT.setWordWrap(True)
        self.CBX_AGREE = QtWidgets.QCheckBox(self.WGT_CENTRAL)
        self.CBX_AGREE.setObjectName("CBX_AGREE")
        self.LYT_FOOTER = QtWidgets.QHBoxLayout()
        self.LYT_FOOTER.setObjectName("LYT_FOOTER")
        SPC_H = QtWidgets.QSpacerItem(
            40, 20, QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Minimum
        )
        self.BTN_SEND = QtWidgets.QPushButton(self.WGT_CENTRAL)
    ...
    def checkForm(self):
        ...
        if not self.OVERFLOW:
            if LENGTH >= self.MAX_INPUT:
                self.OVERFLOW = True
                self.PTE_FEEDBACK.setPlainText(
                    self.PTE_FEEDBACK.toPlainText()[: self.MAX_INPUT]
                )
        else:
            self.OVERFLOW = False
        self.updateAgreementCheck() if LENGTH >= self.MIN_INPUT and LENGTH <= self.MAX_INPUT else self.BTN_SEND.setEnabled(
            0
        )
    ...
    def retranslateUi(self):
        ...
        self.LBL_INSTRUCTION.setText(
            "Please state all your concerns and feedbacks about the application."
        )
        self.LBL_AGREEMENT.setText(
            'By clicking "Send", you agree to include additional details such as system info, crash logs, and other necessary data in the report for the improvements of this application.'
        )
        self.CBX_AGREE.setText(
            "I am sure that all information I have written is accurate"
        )
        self.BTN_SEND.setText("Send")
        self.PTE_FEEDBACK.setPlaceholderText(
            f"Write something at least {self.MIN_INPUT} characters"
        )
    ...
    def sendFeedbackForm(self):
        ...
        MSG_BOX.setIcon(QtWidgets.QMessageBox.Information)
        MSG_BOX.setText(
            f'{"Your feedback was sent successfully. Thank you." if SYS.isOnline() else "It seems like you are not connected to the internet.{nL}Your feedback will be sent whenever available."}'
        )
...
class QWGT_COMPACT(QtWidgets.QMainWindow):
    ...
    def setupUi(self):
        ...
        if CDATA["AlwaysOnTop"] == "True":
            self.setWindowFlags(self.windowFlags() | QtCore.Qt.WindowStaysOnTopHint)
        self.WGT_CENTRAL = QtWidgets.QWidget(self)
        self.WGT_CENTRAL.setObjectName("QWGT_COMPACT")
        self.GRID_MAIN = QtWidgets.QGridLayout(self.WGT_CENTRAL)
        self.GRID_MAIN.setObjectName("gridLayout_2")
        self.gridLayout = QtWidgets.QGridLayout()
        self.gridLayout.setObjectName("gridLayout")
        self.LNE_SEARCH = QtWidgets.QLineEdit(self.WGT_CENTRAL)
        self.LNE_SEARCH.setObjectName("LNE_SEARCH")
        self.LNE_SEARCH.setClearButtonEnabled(True)
        self.LBL_PVW_TITLE = QtWidgets.QLabel(self.WGT_CENTRAL)
        self.LBL_PVW_TITLE.setObjectName("LBL_PVW_TITLE")
        self.BTN_LAUNCH = QtWidgets.QPushButton(self.WGT_CENTRAL)
        self.BTN_LAUNCH.setObjectName("BTN_LAUNCH")
        self.BTN_LAUNCH.setEnabled(0)
    ...
    def switchMode(self):
        ...
        UIA.activateWindow()
        UIC.hide()
        UIA.LNE_SEARCH.setText(
            self.LNE_SEARCH.text()
        )  ## Also passes the entry text to other window
        DELTA = UIC.geometry()
        UIA.move(
            DELTA.x() - 100, DELTA.y() - 100
        )  ## Calculate the change in position so the next window would appear near it.
        CDATA["CompactMode"] = "False"
        CFG.dump()
        global UIZ
        UIZ = UIA
...
class ThreadBackground(QtCore.QThread):
    ...
    def run(self):
        while True:
            ...
            try:
                if not UIZ.LNE_SEARCH.hasFocus() and UIZ == UIA:
                    UIZ.STATUSBAR.showMessage("")

                # UIZ.INP.updateDetails() # <--- This is for real-time update of details

                if UIZ.QUERY_TIMEOUT:
                    if not UIZ.QUERY_TIMEOUT_FILL and int(
                        UIZ.INP.CURRENT_HINFO.num
                    ) in list(range(SYS.HYMNS_MAX + 1)):
                        if (
                            UIZ.LAST_QUERY_ID != UIZ.INP.CURRENT_HINFO.num
                        ):  ## Check if the last query is the same or different
                            ARRAY = [
                                UIZ.INP.CURRENT_HINFO.queries + 1,
                                UIZ.INP.CURRENT_HINFO.launches,
                                UIZ.INP.CURRENT_HINFO.lastAccessed,
                            ]
                            SDATA["DATA"].update(
                                {str(UIZ.INP.CURRENT_HINFO.num): ARRAY}
                            )
                            SDB.dump()  ## Log to Statistic Data
                            UIZ.LAST_QUERY_ID = UIZ.INP.CURRENT_HINFO.num
                        UIZ.QUERY_TIMEOUT = False
                        UIZ.QUERY_TIMEOUT_FILL = 2
                    else:
                        UIZ.QUERY_TIMEOUT_FILL -= 1
...
if __name__ == "__main__":
    ...
    UIZ = (
        UIA if CDATA["CompactMode"] == "False" else UIC
    )  ## 0 = Full Size, 1 = Compact | UIZ = Active Window
